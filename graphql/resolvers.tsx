...
export const resolvers = {
  Query: {
    ...
    // Fetch all bookings for a driver's offers
    getDriverRequests: async (_: any, { driverId }: { driverId: string }) => {
      ...
      return bookings.map((b: any) => ({
        ...
        status: b.status || 'pending',
        createdAt: b.createdAt?.toISOString(),
        offer: offerMap[b.offerId.toString()]
          ? {
              ...offerMap[b.offerId.toString()],
              id: b.offerId.toString(),
              driverName: '',
              createdAt: offerMap[b.offerId.toString()].createdAt?.toISOString(),
            }
          : null,
      }));
    },
    ...
    bookTrip: async (_: any, { offerId, userId, userName, userEmail }: any) => {
      ...
      if (driver?.email && process.env.RESEND_API_KEY) {
        await resend.emails.send({
          ...
          html: `
            <h2>New booking on your trip</h2>
            <p><strong>${userName}</strong> (${userEmail}) has booked your trip.</p>
            <p><strong>Destinations:</strong> ${offer.destinations.join(', ')}</p>
